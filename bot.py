...
DB_NAME = "booking_v6.db"
MAX_BOOKINGS_PER_USER = 5
ADMIN_IDS = {8224923198}  # 
UTC_PLUS_3 = timezone(timedelta(hours=3))

# Если хочешь, чтобы кнопка "Связаться с инструктором" показывала твой контакт:
INSTRUCTOR_CONTACT_TEXT = "@dtdvld33"  # <-- замени на свой @username или телефон
...
def init_db():
    ...
    cur.execute("""
        CREATE TABLE IF NOT EXISTS manual_students (
            ...
            comment TEXT,
            source_status TEXT DEFAULT 'offline',   -- offline / bot
            created_at TEXT DEFAULT CURRENT_TIMESTAMP
        )
    """)
    ...
    cur.execute("""
        CREATE TABLE IF NOT EXISTS slots (
            ...
            booked_source TEXT DEFAULT 'bot',      -- bot/manual
...
def main_menu_keyboard():
    keyboard = [
        ...
        ["👤 Мой профиль", "☎ Связаться с инструктором"],
        ["🏠 Меню"],
    ]
...
def get_bookings_by_date(slot_date: str):
    return db_execute("""
        SELECT slot_time, booked_by_name, booked_by_phone, booked_source
        FROM slots
        WHERE slot_date = ?
          AND is_booked = 1
        ORDER BY slot_time
...
def get_all_slots():
    return db_execute("""
        SELECT slot_date, slot_time, is_booked, booked_by_name, booked_by_phone, booked_source
        FROM slots
        ORDER BY slot_date, slot_time
    """, fetch=True)
...
async def callback_router(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("showdate|"):
        ...
        keyboard = InlineKeyboardMarkup(
            [[InlineKeyboardButton("⬅️ Назад", callback_data="back_slots")]]
        )
    ...
    if data.startswith("bookslot|"):
        ...
        if ok:
            ...
            await query.edit_message_text("Не удалось записаться. Возможно, слот уже занят.")
        return
...
async def today(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    today_str = datetime.now(UTC_PLUS_3).strftime("%Y-%m-%d")
...
async def tomorrow(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    target = (datetime.now(UTC_PLUS_3) + timedelta(days=1)).strftime("%Y-%m-%d")
    rows = get_bookings_by_date(target)

    if not rows:
        await update.message.reply_text("На завтра записей нет.")
        return

    text = f"Записи на завтра ({format_date_ru(target)}):\n\n"
    for t, name, phone, source in rows:
        src = "офлайн" if source == "manual" else "бот"
        phone_text = f" | {phone}" if phone else ""
        text += f"• {t} — {name}{phone_text} | {src}\n"
...
async def week(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    start_date = datetime.now(UTC_PLUS_3).date()
    ...
    for d, t, name, phone, source in rows:
        if d != current_date:
            current_date = d
            text += f"\n{format_date_ru(d)}:\n"
        src = "офлайн" if source == "manual" else "бот"
...
async def allslots(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = "Все слоты:\n\n"
    for d, t, booked, name, phone, source in rows:
        if booked:
            src = "офлайн" if source == "manual" else "бот"
            status = f"занят ({name}"
            if phone:
...
async def genslots_text_flow(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    added = 0
    skipped = 0
    today_date = datetime.now(UTC_PLUS_3).date()
...
async def menu_buttons(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Сначала пропускаем тексты для активных админских сценариев
    ...
    # Если это не кнопка меню — в общий обработчик
    await text_handler(update, context)
...
def main():
    ...
    if app.job_queue:
        ...
        app.job_queue.run_repeating(cleanup_job, interval=21600, first=30, name="cleanup_old")
