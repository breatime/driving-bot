...
def get_slot(slot_date: str, slot_time: str):
    return db_execute("""
        SELECT id, slot_date, slot_time, is_booked, booked_by_user_id, booked_by_name, booked_by_phone, booked_source, finalized
        FROM slots
...
async def noshow(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [
        [InlineKeyboardButton(
            f"{slot_time} — {name}",
            callback_data=f"noshow|{slot_date}|{slot_time}"
        )]
        for slot_date, slot_time, name, phone, source in rows
    ]
