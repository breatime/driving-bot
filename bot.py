...
async def text_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.user_data.get("awaiting_quickslots"):
        if is_admin(user_id):
            ...
            await update.message.reply_text("Этот режим доступен только администратору.", reply_markup=main_menu_keyboard())
        return
...
async def menu_buttons(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Если активен админ-режим ввода слотов, передаем в text_handler
